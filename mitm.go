...
// Handler is a http.Handler that will inject a value
...
func NewHandler(h http.Handler, ln net.Listener, c *tls.Config, closeOnMITM bool) *Handler {
	return &Handler{
		next:        h,
		listener:    newTLSHelloListener(ln, c),
		closeOnMITM: closeOnMITM,
	}
}
...
// newTLSHelloListener returns a new tlsHelloListener that wraps ln.
func newTLSHelloListener(ln net.Listener, config *tls.Config) *tlsHelloListener {
